...
**不是！组件类型和渲染策略是两个不同的概念。**

| 维度         | 说明           | 选项                    |
| ------------ | -------------- | ----------------------- |
| **组件类型** | 代码在哪里运行 | 服务端组件 / 客户端组件 |
| **渲染策略** | HTML 何时生成  | SSG / SSR / ISR / CSR   |

### 服务端组件的渲染策略

| 渲染策略 | 触发条件                                        | 示例                 |
| -------- | ----------------------------------------------- | -------------------- |
| **SSG**  | 没有使用任何动态 API                            | 纯静态页面           |
| **SSR**  | 使用了 `cookies()`、`headers()`、`searchParams` | 需要请求时数据的页面 |
| **ISR**  | 使用了 `revalidate` 配置                        | 定时更新的内容       |

```typescript
// SSG：服务端组件，构建时生成静态 HTML
...
| 渲染策略       | 说明                                        | 示例                 |
| -------------- | ------------------------------------------- | -------------------- |
| **SSG + 水合** | 构建时生成 HTML，浏览器水合后交互           | 静态页面中的交互组件 |
| **SSR + 水合** | 请求时生成 HTML，浏览器水合后交互           | 动态页面中的交互组件 |
| **纯 CSR**     | 只在浏览器渲染（如 `useEffect` 中加载数据） | 图表、实时数据       |

```typescript
// 客户端组件，但首屏 HTML 由 SSG/SSR 生成
'use client';

export default function Counter() {
  const [count, setCount] = useState(0);  // 水合后才生效

  return (
    <button onClick={() => setCount(count + 1)}>
      {count}  {/* 首屏 HTML 已有，但交互需水合 */}
    </button>
  );
}
```
...
页面请求
    │
    ├─ 服务端组件
    │   ├─ 无动态 API → SSG（构建时生成 HTML）
    │   ├─ 有 cookies()/headers() → SSR（请求时生成 HTML）
    │   └─ 有 revalidate → ISR（定时重新生成）
    │
    └─ 客户端组件
        ├─ 被 SSG 页面引用 → SSG HTML + 浏览器水合
        ├─ 被 SSR 页面引用 → SSR HTML + 浏览器水合
        └─ useEffect 中获取数据 → CSR（浏览器渲染）
```
...
| 误区               | 正确理解                                   |
| ------------------ | ------------------------------------------ |
| "客户端组件 = CSR" | 客户端组件也可以有 SSG/SSR 的首屏 HTML     |
| "服务端组件 = SSG" | 服务端组件也可以是 SSR/ISR                 |
| "水合 = CSR"       | 水合是让已有 HTML 变得可交互，不是重新渲染 |
...
```typescript
// 服务端生成的 HTML（已有内容）
<button>点击 0 次</button>

// 水合后（变得可交互）
<button onClick={...}>点击 0 次</button>
```
...
| 文件类型           | 是否打包到客户端 | 说明                     |
| ------------------ | ---------------- | ------------------------ |
| 服务端组件         | ❌ 不打包        | 只输出 HTML + RSC 元数据 |
| 客户端组件         | ✅ 打包          | 完整的 JS 代码           |
| 被客户端引用的文件 | ✅ 打包          | 包括依赖的所有模块       |

### 示例分析

```typescript
// app/page.tsx（服务端组件）
import HeavyLib from 'heavy-lib';        // ❌ 不打包到客户端
import ClientComp from './ClientComp';   // ✅ 打包到客户端
import utils from '@/lib/utils';         // ❌ 不打包（只被服务端使用）

export default function Page() {
  const data = HeavyLib.process();       // 只在服务端运行
...
| 代码                  | 服务端组件中 | 客户端组件中 |
| --------------------- | ------------ | ------------ |
| `import { useState }` | ❌ 报错      | ✅ 打包      |
| `import { cookies }`  | ✅ 不打包    | ❌ 报错      |
| `import HeavyLib`     | ✅ 不打包    | ✅ 打包      |
| `import utils`        | ✅ 不打包    | ✅ 打包      |
...
- **First Load JS** = 客户端组件的 JS 大小
- 服务端组件的代码**不会**出现在这个数字中

### 浏览器能看到什么？

| 组件类型   | 浏览器下载的 JS | 浏览器看到的 HTML                  |
| ---------- | --------------- | ---------------------------------- |
| 服务端组件 | ❌ 无           | ✅ 完整 HTML                       |
| 客户端组件 | ✅ 完整 JS      | ✅ 首屏 HTML（SSG/SSR）或空（CSR） |

### 打包优化建议

| 策略               | 说明                 | 效果             |
| ------------------ | -------------------- | ---------------- |
| 服务端优先         | 尽量用服务端组件     | 减少 JS 体积     |
| 客户端组件尽量小   | 只包含必要的交互逻辑 | 加快加载速度     |
| 动态导入           | `import()` 延迟加载  | 按需加载 JS      |
| 避免客户端引用大库 | 大库放在服务端       | 显著减少打包体积 |
